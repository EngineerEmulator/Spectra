...
public class SpectraBean {
    ...
    public void saveBlogPost() throws SQLException {
        ...
        try {
            ...
            setErrorMessage("Blog ekleme başarıyla gerçekleşti.");
            ps.executeUpdate();
        } catch (Exception ex) {
            System.out.println(ex.toString());
            setErrorMessage("Blog ekleme başarısız oldu.");
        } finally {
            connection.close();
        }
    }
    ...
    public void saveUserMessages() throws SQLException {
        ...
        try {
            ...
            ps = connection.prepareStatement("insert into UserMessages "
                    ...
                    + "values (?,?,?,?,?,0)");
            //Sql query'e Parametreler veriliyor.
            ps.setString(1, getUserMessage());
            ps.setString(2, getName());
            ps.setString(3, getSurname());
            ps.setString(4, getEmail());
            ps.setTimestamp(5, new Timestamp(System.currentTimeMillis()));
            //Query çalıştırılıyor. kullanıcının mesajını usermessages tablosuna kaydetmiş oluyoruz.
            ps.executeUpdate();
            setErrorMessage("Mesajınız başarıyla gönderildi.");
        } catch (Exception ex) {
            System.out.println(ex.toString());
        } finally {
            connection.close();
    ...
    public ResultSet fetchBlogPostsForUsers() throws SQLException {
        ...
        try {
            ...
            //row set oluşturulup database'den gelen değerler içerisine konularak ön yüze döndürülüyor.
            rowSet = new com.sun.rowset.CachedRowSetImpl();
            rowSet.populate(ps.executeQuery());
        } catch (Exception ex) {
            System.out.println(ex.toString());
    ...
    public void editBlogPosts(String blogId) throws SQLException {
        ...
        try {
            ...
            ps.setInt(4, BlogId);
            ps.executeUpdate();
            setErrorMessage("Güncelleme başarıyla yapıldı.");
        } catch (Exception ex) {
            System.out.println(ex.toString());
        ...
        try {
            ...
            if (getUsername() != null && !"".equals(getUsername()) && getPassword() != null && !"".equals(getPassword())) {
                ...
                if (rowSet.first()) {
                    ...
                    setErrorMessage("Giriş başarısız oldu. Doğru kullanıcı adı ve şifrenizle tekrar giriş yapmayı deneyiniz.");
                    setIsLoggedIn(false);
                    FacesContext.getCurrentInstance().getExternalContext().redirect(url);
                }
